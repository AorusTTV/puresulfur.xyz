// src/components/…/CreateBattleDialog.tsx  – REWRITTEN  
...
export const CreateBattleDialog: React.FC<Props> = ({
  ...
  const handleStartBattle = async () => {
    ...
    const battleCrates = basketArr.map(({ crate, qty }) => ({ crate, quantity: qty }));

    await createBattle(rustCrates[0], onBattleCreated, battleCrates); // (api expects first crate arg, keep signature)
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {/* ------------ BattleConfigForm embeds CrateGrid & summary ------------- */}
        <BattleConfigForm
        ...
        <div className="flex justify-end space-x-3 mt-6">
          ...
          <Button
            disabled={basketArr.length === 0 || !profile || profile.balance < totalCost}
