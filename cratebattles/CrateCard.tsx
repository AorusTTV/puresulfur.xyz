...
export const CrateCard: React.FC<Props> = ({
  ...
  const inspect = (evt?: React.MouseEvent) => {
    /* ① stop the original pointer event so Radix never sees it */
    evt?.stopPropagation();
    (evt?.nativeEvent as PointerEvent | undefined)?.stopImmediatePropagation?.();

    /* ② freeze outside pointer-events while SweetAlert is open */
    ...
    Swal.fire({
      ...
      /* ③ restore normal clicks after popup closes */
      htmlRoot.style.removeProperty('pointer-events');
    });
  };
    ...
    >
      ...
      <div className="action-row" onClick={(e) => e.stopPropagation()}>
        ...
        <button
          ...
          title="Inspect crate"
          onClick={(e) => inspect(e)}   /* pass event */
        >
          <svg
            width="18"
            height="18"
